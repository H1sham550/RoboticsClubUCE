...
// Certificates Database loaded asynchronously from certificates.json on demand
let certDatabaseCache = null;

async function loadCertificatesDatabase() {
    if (certDatabaseCache) return certDatabaseCache;
    try {
        const res = await fetch('certificates.json');
        if (res.ok) {
            certDatabaseCache = await res.json();
            return certDatabaseCache;
        }
    } catch (err) {
...
async function verifyCertificate() {
    ...
    if (query.length < 3) {
        if (errorText) errorText.textContent = "Please enter at least 3 characters of your name to search (e.g. Kevin).";
        errorBox.classList.remove('hidden');
        return;
    }

    const certDatabase = await loadCertificatesDatabase();
